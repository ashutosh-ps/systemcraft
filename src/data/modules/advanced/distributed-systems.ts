...
1. **The network is reliable.** Packets drop. AWS publishes 99.99% availability for in-region connectivity — that missing 0.01% is ~4.3 minutes/month of weirdness, *by SLA*.
...
3. **Bandwidth is infinite.** A 10 Gbps NIC moves ~1.2 GB/s — shuffling a 10 TB dataset across it takes 2.3 hours, ignoring everything else on the link.
...
2. A stalls — a 15-second GC pause, real and documented in production (HBase hit exactly this with ZooKeeper locks).
  ...
  quiz: [
    ...
    },
    {
      ...
      options: [
        ...
        'Clients that failed simultaneously would otherwise retry simultaneously — jitter decorrelates the synchronized waves',
      ],
      answer: 3,
      explanation:
    ...
    },
    {
      ...
      options: [
        'Writes from A that are actually older can overwrite genuinely newer writes from B — silent data loss',
      ...
      answer: 0,
      explanation:
        'LWW trusts timestamps to encode real order, but 30 ms of skew exceeds many write intervals — A\'s stale write carries a "newer" timestamp and wins. No error is raised anywhere; the data is just gone. Vector clocks (or avoiding LWW) fix this.',
    },
    {
