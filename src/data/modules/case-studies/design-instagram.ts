  ...
  sections: [
    {
      ...
      md: `
Never start drawing boxes. Spend the first 5 minutes scoping, because interviewers grade this as heavily as the design.
...
`,
    ...
    },
    {
      ...
      diagram: {
        ...
        nodes: [
          ...
          { id: 'app', label: 'App Servers', kind: 'server', x: 410, y: 330, detail: 'Stateless API tier: auth, validation, orchestration. At ~1K QPS/node, peak needs ~175 nodes plus 30% headroom (~230). Autoscaled.' },
    ...
    {
      type: 'text',
      title: 'Step 4: The core problem of generating the feed',
      md: `
...
~200 users' data on dozens of shards**, easily 50–500 ms of tail-latency pain, and that is at 175K QPS peak. The p99 < 200 ms
...
followers, so push is cheap. Celebrities have huge audiences but are few in number (~thousands of accounts), and their
recent posts sit in cache so the read-time merge touches maybe 1–5 of them per user.
...
2. Client uploads the 2 MB original **directly to S3**, so the app tier never proxies photo bytes (at 3.5K uploads/sec that would be ~7 GB/s of pointless traffic through your servers).
...
> Strong closing move: state what you'd build first (push-only MVP, no celebrity path) and which metric, celebrity
...
`,
  ...
  interviewQuestions: [
    ...
    },
    {
      ...
      hint: 'Because timelines store only IDs, mark the post deleted in the post store/cache and filter at hydration time, with no need to touch 5,000 lists synchronously. Lazy removal on next timeline rebuild. Mention the CDN: invalidate or rely on hashed immutable URLs becoming unreachable.',
      difficulty: 'Junior',
    },
  ],
  commonMistakes: [
    'Jumping straight to architecture without stating the read:write ratio. Every major decision (fan-out, caching, CDN) is justified by ~100:1, so say the number first.',
    'Storing photos in the database. Media is ~5,000× the metadata volume (250 TB/day vs ~50 GB), so it belongs in object storage with CDN delivery, full stop.',
