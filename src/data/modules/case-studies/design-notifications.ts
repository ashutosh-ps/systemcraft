...
- **Latency tiers**: OTP/security ≤ 5 s; transactional ≤ 30 s; marketing — minutes are fine. Priority must be explicit in the system.
...
`,
    ...
    },
    {
      ...
      md: `
At-least-once delivery is the only honest guarantee a queue gives you, so **duplicates are guaranteed by design** —
a producer retries a timed-out API call, a worker crashes after sending but before committing its offset, Kafka
...
verifies status instead of re-sending. This narrows exactly-once to "exactly-once *effect*" — the standard, achievable
target.
...
quiet hours — but never bypass legal opt-outs for the channel.
...
instead of sending; a scheduler flushes it on a trigger — count threshold (10 events), time window (hourly), or smart
timing (user's historically active hour). One rendered notification: **"Anna and 39 others liked your post."**
...
requirements: store and resolve the **user's timezone** (a 03:00 UTC send is 19:00 in California and 04:00 in Berlin —
per-user, not per-region), and on release, **smear** the held queue over 15–30 minutes instead of firing the whole
...
`,
    },
    {
      ...
      numbers: [
        ...
        { metric: 'Push delivery latency', value: 'p50 ~0.5–2 s', context: 'APNs/FCM to a reachable device — and the API itself is free.' },
