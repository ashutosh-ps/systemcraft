...
- **Trip lifecycle**: requested → matched → driver en route → in progress → completed → paid. Each transition must be durable — money depends on it.
...
- **Location updates every 4–5 s** from every active driver. At ~5M concurrently active drivers worldwide that's **>1M writes/second** — your write path is the beast, not the read path.
...
`,
    ...
    {
      type: 'text',
      title: 'Step 3: The core problem — geospatial indexing',
      md: `
...
the correct failure mode for a dead phone. Memory math: 5M drivers × ~100 bytes ≈ **500 MB** — the entire world's
...
- **Honoring the quote.** The multiplier is stamped into the fare quote and held for ~2 minutes (quote ID stored
  with its multiplier). The rider pays what they were shown even if surge ticks up before they confirm — pricing
...
`,
  ...
  interviewQuestions: [
    ...
    },
    {
      ...
      hint: 'Sequential greedy matching stops working — you need windowed batch matching: hold requests for a few seconds, then solve a small assignment problem over candidate route overlaps. Discuss the latency-vs-efficiency trade and how trip state machines get more complex (multiple pickups, re-routing mid-trip).',
      difficulty: 'Senior',
    },
    {
