...
- **Buffering spikes.** A flash sale pushes 12,000 orders/min at a payment processor that sustains 3,000/min. Synchronously, 75% of requests fail. With a queue in between, the backlog grows for a few minutes and drains — every order eventually processes.
...
storage (it's a log, not a queue), but a slow consumer still falls behind — monitor **consumer lag** per group.
`,
    },
    {
      type: 'text',
...
`,
  ...
  quiz: [
    ...
    },
    {
      ...
      options: ['20 — Kafka load-balances messages across all of them', '1 — only the group leader consumes', '16 — the rest sit idle', '4 — one per broker'],
      answer: 2,
      explanation:
