...
- **Use when:** read-after-write consistency matters — user profile edits, settings, shopping carts.
...
- **Replicate the key**: write \`config:flag:1\` … \`config:flag:10\` and have clients read a random suffix —
...
- **L1 in-process cache** for the hottest few hundred keys: even a 1-second local TTL absorbs almost all reads
  for a viral key.
- **Detect first**: sample commands (\`redis-cli --hotkeys\`, or client-side metrics) — hot keys are invisible in
...
`,
    },
    {
      ...
      scenario: {
        ...
        afterDescription:
          'Two cache.r7g.large Redis nodes ($240/month) with 5-minute jittered TTLs and single-flight loading. Hit ratio stabilizes at 95%, so Postgres sees ~400 QPS of misses plus writes — it was downsized one instance class.',
        metrics: [
  ...
  quiz: [
    ...
    },
    {
      ...
      answer: 0,
      explanation:
        'Write-behind acks after writing only the cache and flushes to the DB asynchronously — anything buffered but not yet flushed dies with the node. Use it only for tolerable-loss data or with a durable buffer.',
    },
    {
      question:
