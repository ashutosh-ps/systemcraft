  ...
  sections: [
    {
      ...
      md: `
You can refactor an internal function tomorrow. A public API endpoint is forever: once a single paying customer
integrates it, it is effectively immutable, and Stripe still serves API versions from **2014**. So API design is the
one place where upfront care pays compounding interest.
...
- \`POST /orders\`: create. Not idempotent (the dangerous one, covered more below).
...
2. **It skips or duplicates under writes.** Insert one row while a client paginates and every subsequent page shifts by one, so item 101 shows up twice, or never.
...
needs to be fast and shared: Redis with TTL, or a DB table with a unique index, which is what makes the "first time"
check race-free when two retries arrive concurrently.
...
horizontally scaled, multi-AZ tier, since it's stateless), and the gateway becoming a **dumping ground for business
...
GitHub Apps on Enterprise Cloud); unauthenticated calls get just 60/hour. That 83× gap pushes everyone
...
- **5xx, the server failed and retrying may help**: \`500\` bug, \`502/504\` upstream dead or slow, \`503\` overloaded.
